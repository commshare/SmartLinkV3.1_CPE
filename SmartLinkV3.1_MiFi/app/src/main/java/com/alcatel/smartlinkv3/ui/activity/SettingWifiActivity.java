...
public class SettingWifiActivity extends BaseFragmentActivity implements OnClickListener {
    ...
    private void onWifModeChanged() {
        m_nPreWlanAPMode = WlanFrequency.antiBuild(BusinessMannager.getInstance().getWlanFrequency());
        ...
        if (WlanFrequency.antiBuild(WlanFrequency.Frequency_24GHZ) == m_nWlanAPMode) {
            ...
            if (SecurityMode.Disable == mode) {
                ...
                m_nPreType = WEPEncryption.antiBuild(BusinessMannager.getInstance().getWEPEncryption());
            } else {
                m_nPreType = WPAEncryption.antiBuild(BusinessMannager.getInstance().getWPAEncryption());
                if (SecurityMode.Disable == mode) {
                    m_strPreKey = "";
                } else {
            ...
            if (SecurityMode.Disable == mode) {
                ...
                m_nPreType = WEPEncryption.antiBuild(BusinessMannager.getInstance().getWEPEncryption_5G());
            } else {
                m_nPreType = WPAEncryption.antiBuild(BusinessMannager.getInstance().getWPAEncryption_5G());
                if (SecurityMode.Disable == mode) {
                    m_strPreKey = "";
                } else {
    ...
    private void setWlanSettingItems() {
        ...
        BusinessMannager.getInstance().sendRequestMessage(MessageUti.WLAN_SET_WLAN_SETTING_REQUSET, data);
        ShowWaiting(true);
    }
