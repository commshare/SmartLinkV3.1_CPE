...
public class WlanManager extends BaseManager {
    ...
    private void startGetHostNumTask() {
        if (FeatureVersionManager.getInstance().isSupportApi("Wlan", "GetNumOfHosts") != true)
            return;
        if (m_getHostNumTask == null) {
    ...
    public void getWlanSetting(DataValue data) {
        if (FeatureVersionManager.getInstance().isSupportApi("Wlan", "GetWlanSettings") != true)
            return;

        HttpRequestManager.GetInstance().sendPostRequest(new HttpWlanSetting.GetWlanSetting("5.4", new IHttpFinishListener() {
            @Override
            public void onHttpRequestFinish(BaseResponse response) {
                ...
                Intent megIntent = new Intent(MessageUti.WLAN_GET_WLAN_SETTING_REQUSET);
    ...
    public void setWlanSetting(DataValue data) {
        if (FeatureVersionManager.getInstance().isSupportApi("Wlan", "SetWlanSettings") != true)
            return;
        ...
        HttpRequestManager.GetInstance().sendPostRequest(new HttpWlanSetting.SetWlanSetting("5.5", settings, new IHttpFinishListener() {
            @Override
            public void onHttpRequestFinish(BaseResponse response) {
                ...
                Intent megIntent = new Intent(MessageUti.WLAN_SET_WLAN_SETTING_REQUSET);
                megIntent.putExtra(MessageUti.RESPONSE_RESULT, ret);
                megIntent.putExtra(MessageUti.RESPONSE_ERROR_CODE, strErrcode);
                m_context.sendBroadcast(megIntent);
            }
    ...
    public void SetWPSPin(DataValue data) {
        if (FeatureVersionManager.getInstance().isSupportApi("Wlan", "SetWPSPin") != true)
            return;

        String m_strPin = (String) data.getParamByKey("WpsPin");

        HttpRequestManager.GetInstance().sendPostRequest(new HttpWlanSetting.SetWPSPin("5.6", m_strPin, new IHttpFinishListener() {
            @Override
            public void onHttpRequestFinish(BaseResponse response) {
                ...
                Intent megIntent = new Intent(MessageUti.WLAN_SET_WPS_PIN_REQUSET);
                megIntent.putExtra(MessageUti.RESPONSE_RESULT, ret);
                megIntent.putExtra(MessageUti.RESPONSE_ERROR_CODE, strErrcode);
                m_context.sendBroadcast(megIntent);
            }
    ...
    public void SetWPSPbc(DataValue data) {
        if (FeatureVersionManager.getInstance().isSupportApi("Wlan", "SetWPSPbc") != true)
            return;

        HttpRequestManager.GetInstance().sendPostRequest(new HttpWlanSetting.SetWPSPbc("5.7", new IHttpFinishListener() {
            @Override
            public void onHttpRequestFinish(BaseResponse response) {
    ...
    public void getWlanSupportMode(DataValue data) {
        ...
        if (blCPEWifiConnected) {
            HttpRequestManager.GetInstance().sendPostRequest(new HttpWlanSetting.getWlanSupportModeRequest("5.8", new IHttpFinishListener() {
