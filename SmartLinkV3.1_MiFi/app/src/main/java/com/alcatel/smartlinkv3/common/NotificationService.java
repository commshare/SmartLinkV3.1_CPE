...
public class NotificationService extends Service {
    ...
    public void onCreate() {
        ...
        this.registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.STATISTICS_SET_MONTHLY_PLAN_REQUSET));
        this.registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.STATISTICS_CLEAR_ALL_RECORDS_REQUSET));
        this.registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.STATISTICS_GET_USAGE_SETTINGS_ROLL_REQUSET));


        this.registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.UPDATE_GET_DEVICE_NEW_VERSION));
        this.registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.POWER_GET_BATTERY_STATE));
        this.registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.SIM_GET_SIM_STATUS_ROLL_REQUSET));
        this.registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.CPE_CHANGED_ALERT_SWITCH));


        m_nm = (NotificationManager) this.getSystemService(Context.NOTIFICATION_SERVICE);
    }
    ...
    private static boolean isServiceRunning(Context context) {
        ...
        for (RunningServiceInfo info : infos) {
            if (info.service.getClassName().equalsIgnoreCase(NotificationService.class.getName())) {
                bRunning = true;
                break;
            }
        }
    ...
    private void alert() {
        ...
        if (settings.HMonthlyPlan > 0 && m_isNeedToAlertUsageLimit == true && (((m_UsageRecordResult.HUseData >= settings.HMonthlyPlan) && m_AlertUsageLimitOverOneTime == true) || (m_UsageRecordResult.HUseData < settings.HMonthlyPlan && isOverMonthlyPlan(settings, m_UsageRecordResult) && (m_AlertUsageLimitLessOneTime == true)))) {
        ...
        } else if (m_isNeedToAlertBatteryLimit == true && ((isOverBattery2(batteryinfo.getBatterLevel()) && m_AlertBatteryLimit2OneTime == true) || ((isOverBattery1(batteryinfo.getBatterLevel())) && (m_AlertBatteryLimit1OneTime == true)))) {
        ...
        } else if (isHaveNewVersion(newVersioninfo.getState()) && m_isNeedToAlertUpgrade == true && m_AlertUpgradeOneTime == true) {
