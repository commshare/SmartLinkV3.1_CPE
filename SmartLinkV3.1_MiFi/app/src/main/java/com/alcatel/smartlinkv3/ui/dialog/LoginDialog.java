...
/*	private static boolean m_bAlreadyLogin = false;
    ...
    private class AuthenficationBroadcastReviever extends BroadcastReceiver {
        ...
        public void onReceive(Context arg0, Intent arg1) {
            ...
            if (arg1.getAction().equalsIgnoreCase(MessageUti.SIM_GET_SIM_STATUS_ROLL_REQUSET)) {
                boolean bCPEWifiConnected = DataConnectManager.getInstance().getCPEWifiConnected();
                if (!bCPEWifiConnected) {
                    closeDialog();
                }
            } else if (arg1.getAction().equalsIgnoreCase(MessageUti.USER_LOGIN_REQUEST)) {
                m_bIsApply = false;
                ...
                } else if (BaseResponse.RESPONSE_OK == nRet && strErrorCode.equalsIgnoreCase(ErrorCode.ERR_USER_OTHER_USER_LOGINED)) {
                ...
                } else if (BaseResponse.RESPONSE_OK == nRet && strErrorCode.equalsIgnoreCase(ErrorCode.ERR_LOGIN_TIMES_USED_OUT)) {
    ...
    private void createDialog() {
        ...
        m_strMsgOtherUserLogined = m_context.getResources().getString(R.string.login_other_user_logined_error_forcelogin_msg);
    ...
    public static boolean isLoginSwitchOff() {
        String strLoginFile = Environment.getExternalStorageDirectory().getAbsolutePath() + "/CPE/LoginDisable";
