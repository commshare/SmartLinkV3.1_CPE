...
public class SettingDeviceActivity extends BaseActivity implements OnClickListener {
    ...
    private void initUi() {
        ...
        isPinRequired = false;
        if (BusinessMannager.getInstance().getSimStatus().m_PinState == ENUM.PinState.PinEnableVerified || BusinessMannager.getInstance().getSimStatus().m_PinState == ENUM.PinState.EnableButNotVerified || BusinessMannager.getInstance().getSimStatus().m_PinState == ENUM.PinState.RequirePUK) {
    ...
    private void simRollRequest() {
        ...
        if (isPinRequired && sim.m_nPinRemainingTimes > 0) {
            ...
            final InquireReplaceDialog inquireDlg = new InquireReplaceDialog(SettingDeviceActivity.this);
            inquireDlg.setCancelDisabled();
            inquireDlg.m_titleTextView.setText(R.string.dialog_warning_title);
            inquireDlg.m_contentTextView.setText(R.string.dialog_warning_error_pin_code_error_3times);
        ...
        switch (nID) {
            ...
            case R.id.setting_device_pincode_editor:
                ...
                if (simStatus.m_PinState == PinState.EnableButNotVerified) {
                    ...
                    if (simStatus.m_SIMState == SIMState.Accessable || simStatus.m_SIMState == SIMState.PinRequired || simStatus.m_SIMState == SIMState.PukRequired) {
    ...
    private void onBtnPincodeSetting() {
        ...
        if (BusinessMannager.getInstance().getSimStatus().m_SIMState == SIMState.NoSim || BusinessMannager.getInstance().getSimStatus().m_SIMState == SIMState.InvalidSim || BusinessMannager.getInstance().getSimStatus().m_SIMState == SIMState.SimCardIsIniting) {
            String strInfo = getString(R.string.home_sim_not_accessible);
            Toast.makeText(this, strInfo, Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void onBtnPowerOff() {
        BusinessMannager.getInstance().sendRequestMessage(MessageUti.SYSTEM_SET_DEVICE_POWER_OFF, null);
    }
    ...
    protected void onResume() {
        m_bNeedBack = false;
        super.onResume();
        registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.SYSTEM_SET_DEVICE_REBOOT));
        registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.SYSTEM_SET_DEVICE_RESET));
        registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.SYSTEM_SET_DEVICE_POWER_OFF));
        registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.SYSTEM_GET_SYSTEM_INFO_REQUSET));
        ...
        this.registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.SIM_CHANGE_PIN_STATE_REQUEST));
        this.registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.USER_LOGOUT_REQUEST));
        this.registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.UPDATE_SET_DEVICE_STOP_UPDATE));
        this.registerReceiver(m_msgReceiver, new IntentFilter(MessageUti.UPDATE_GET_DEVICE_NEW_VERSION));
    ...
    protected void onBroadcastReceive(Context context, Intent intent) {
        ...
        Logger.t("ma_device").v("onBroadcastReceive:" + intent.getAction());
        ShowWaiting(false);
        ...
        } else if (intent.getAction().equalsIgnoreCase(MessageUti.SIM_CHANGE_PIN_STATE_REQUEST)) {
            int nResult = intent.getIntExtra(MessageUti.RESPONSE_RESULT, BaseResponse.RESPONSE_OK);
            String strErrorCode = intent.getStringExtra(MessageUti.RESPONSE_ERROR_CODE);
            if (BaseResponse.RESPONSE_OK == nResult && strErrorCode.length() == 0) {
