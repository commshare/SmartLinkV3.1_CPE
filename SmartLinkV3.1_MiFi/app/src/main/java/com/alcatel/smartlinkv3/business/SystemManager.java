...
public class SystemManager extends BaseManager {
    ...
    class GetFeaturesTask extends TimerTask {
        @Override
        public void run() {
            HttpRequestManager.GetInstance().sendPostRequest(new HttpSystem.GetFeature("16.1", new IHttpFinishListener() {
                @Override
                public void onHttpRequestFinish(BaseResponse response) {
                    ...
                    Intent megIntent = new Intent(MessageUti.SYSTEM_GET_FEATURES_ROLL_REQUSET);
    ...
    public void getSystemInfo(DataValue data) {
        if (FeatureVersionManager.getInstance().isSupportApi("System", "GetSystemInfo") != true)
            return;

        boolean bCPEWifiConnected = DataConnectManager.getInstance().getCPEWifiConnected();
        if (bCPEWifiConnected) {
            HttpRequestManager.GetInstance().sendPostRequest(new HttpSystem.GetSystemInfo("13.1", new IHttpFinishListener() {
                @Override
                public void onHttpRequestFinish(BaseResponse response) {
                    int ret = response.getResultCode();
                    ...
                    Intent megIntent = new Intent(MessageUti.SYSTEM_GET_SYSTEM_INFO_REQUSET);
                    megIntent.putExtra(MessageUti.RESPONSE_RESULT, ret);
                    megIntent.putExtra(MessageUti.RESPONSE_ERROR_CODE, strErrcode);
                    m_context.sendBroadcast(megIntent);
                }
    ...
    class GetSystemStatusTask extends TimerTask {
        @Override
        public void run() {
            HttpRequestManager.GetInstance().sendPostRequest(new HttpSystem.GetSystemStatus("13.4", new IHttpFinishListener() {
                @Override
                public void onHttpRequestFinish(BaseResponse response) {
                    int ret = response.getResultCode();
                    String strErrcode = response.getErrorCode();
                    ...
                    Intent megIntent = new Intent(MessageUti.SYSTEM_GET_SYSTEM_STATUS_REQUSET);
                    megIntent.putExtra(MessageUti.RESPONSE_RESULT, ret);
                    megIntent.putExtra(MessageUti.RESPONSE_ERROR_CODE, strErrcode);
                    m_context.sendBroadcast(megIntent);
                }
    ...
    public void rebootDevice(DataValue data) {
        ...
        if (blWifiConnected) {
            HttpRequestManager.GetInstance().sendPostRequest(new HttpSystem.SetDeviceReboot("13.5", new IHttpFinishListener() {
    ...
    public void resetDevice(DataValue data) {
        ...
        boolean blWifiConnected = DataConnectManager.getInstance().getCPEWifiConnected();
        if (blWifiConnected) {
            HttpRequestManager.GetInstance().sendPostRequest(new HttpSystem.SetDeviceReset("13.6", new IHttpFinishListener() {
    ...
    public void backupDevice(DataValue data) {
        ...
        boolean blWifiConnected = DataConnectManager.getInstance().getCPEWifiConnected();
        if (blWifiConnected) {
            HttpRequestManager.GetInstance().sendPostRequest(new HttpSystem.SetDeviceBackup("13.7", new IHttpFinishListener() {
    ...
    public void restoreDevice(DataValue data) {
        ...
        if (blWifiConnected) {
            ...
            HttpRequestManager.GetInstance().sendPostRequest(new HttpSystem.SetDeviceRestore("13.8", strFile, new IHttpFinishListener() {
    ...
    public void setDevicePowerOff(DataValue data) {
        ...
        boolean blWifiConnected = DataConnectManager.getInstance().getCPEWifiConnected();
        if (blWifiConnected) {
            HttpRequestManager.GetInstance().sendPostRequest(new HttpSystem.setDevicePowerOffRequest("13.9", new IHttpFinishListener() {
    ...
    public void setAppBackup(DataValue data) {
        ...
        boolean blWifiConnected = DataConnectManager.getInstance().getCPEWifiConnected();
        if (blWifiConnected) {
            HttpRequestManager.GetInstance().sendPostRequest(new HttpSystem.setAppBackupRequest("13.10", new IHttpFinishListener() {
    ...
    public void setAppRestoreBackup(DataValue data) {
        ...
        boolean blWifiConnected = DataConnectManager.getInstance().getCPEWifiConnected();
        if (blWifiConnected) {
            HttpRequestManager.GetInstance().sendPostRequest(new HttpSystem.setAppRestoreBackupRequest("13.11", new IHttpFinishListener() {
                ...
                public void onHttpRequestFinish(BaseResponse response) {
                    ...
                    int nRet = response.getResultCode();
                    String strError = response.getErrorCode();
                    if (nRet == BaseResponse.RESPONSE_OK && strError.length() == 0) {
