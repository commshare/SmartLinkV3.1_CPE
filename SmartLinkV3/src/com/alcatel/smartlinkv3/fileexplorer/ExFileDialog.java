...
class FileDialog extends Activity {
        ...
        if (mDir.equals(PATH_SD_CARD)) {
            ...
            if (dirs.length > DIR_LIMIT) {
                ...
                for (int i = 0; i < DIR_LIMIT; i++) {
                    textContent = File.separator +  dirs[dirs.length - 1 - i] + textContent;
                }
    ...
    private void setResultCodeBuilder() {
        this.mResultCodeBuilder = ResultCode.getBuilder().
                ...
                setSucceedDesc(getResources().getStringArray(R.array.succeed_desc)).
            ...
            if (args.length > 0) {
                try {
                    ...
                    if (checkFileName(filePath2Name(folerPath)))
                        getResultCode(ResultCode.FAIL_FOLDER_EXISTS).doResult();
    ...
    private void showNewFolerDia (Context context) {
        AlertDialog.Builder customDia = new AlertDialog.Builder(context);
        
        final View viewDia = LayoutInflater.from(context).inflate(R.layout.new_foler_dialog, null);  
        customDia.setTitle(R.string.new_foler);
        customDia.setView(viewDia);
        customDia.setPositiveButton(R.string.confirm, new DialogInterface.OnClickListener() {
              
            @Override
            public void onClick(DialogInterface dialog, int which) {
                EditText diaInput= (EditText) viewDia.findViewById(R.id.txt_dia_input);
                
                String folerNameLabel = null;
                try {
                    folerNameLabel = viewDia.getContext().getResources().getString(R.string.foler_name);
                } catch (NotFoundException e) {
                    folerNameLabel = "foler name";
                }
                String folerName = diaInput.getText().toString();
                String folerPath = mDir + File.separator + folerName;
                showMessage("Add foler", folerNameLabel + " is " + folerPath);
    ...
    void showMessage(String tag, String message) {
        Toast toast = Toast.makeText(this, tag + " : " + message, Toast.LENGTH_SHORT);
        toast.setGravity(android.view.Gravity.BOTTOM | android.view.Gravity.FILL_HORIZONTAL, 0, 0);
        toast.show();
    }
}
