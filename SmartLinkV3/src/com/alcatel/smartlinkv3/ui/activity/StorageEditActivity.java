...
public class StorageEditActivity extends BaseActivity implements OnClickListener {
	...
	public static String EDIT_SELECT_FILES = "com.alcatel.cpe.ui.activity.selectedFiles";
    ...
    private Handler m_smbListFilesTaskHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
    		...
    		case SmbUtils.SMB_MSG_TASK_FINISH:
    			...
    			m_editStorageListData = (ArrayList<FileItem>) res.mFiles.clone();
    		...
    		case SmbUtils.SMB_MSG_TASK_ERROR:
    			m_progressWaiting.setVisibility(View.GONE);			
    			String strErr = StorageEditActivity.this.getString(R.string.list_samba_files_failed);
    			Toast.makeText(StorageEditActivity.this, strErr, Toast.LENGTH_LONG).show();
    			break;   			
    ...
    private Handler m_smbRenameFileTaskHandler = new Handler(){
    	public void handleMessage(Message msg) {
    		...
    		case SmbUtils.SMB_MSG_TASK_ERROR:
    			String msgError = StorageEditActivity.this.getString(R.string.rename_failed);
    			Toast.makeText(StorageEditActivity.this, msgError, Toast.LENGTH_SHORT).show();
    			StorageEditActivity.this.finish();
    			break;
    			
    		case SmbUtils.SMB_MSG_FILE_EXISTS:
    			String msgExists = StorageEditActivity.this.getString(R.string.rename_is_exist);
    			Toast.makeText(StorageEditActivity.this, msgExists, Toast.LENGTH_SHORT).show();		
    			break;
    		}  		
    	}    	
    };
			...
			case SmbUtils.SMB_MSG_TASK_FINISH:
				...
				if(res.failed != 0) {
					...
					if(res.succeeded + res.failed == 1) {
						msgRes = StorageEditActivity.this.getString(R.string.delete_file_fail1);
					}else{
						String str = StorageEditActivity.this.getString(R.string.delete_file_fail2);
						msgRes = String.format(str, res.succeeded, res.failed);
					}
					Toast.makeText(StorageEditActivity.this, msgRes, Toast.LENGTH_SHORT).show();
				}
				Intent intent = new Intent(SmbUtils.SMB_MSG_REFRESH_FILES);			
				StorageEditActivity.this.sendBroadcast(intent);					
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		m_strCurLocation = this.getIntent().getStringExtra(LocalStorageActivity.FLAG_CURRENT_LOCATION);
		m_curDirectory = this.getIntent().getStringExtra(LocalStorageActivity.CURRENT_DIRECTORY);
		...
		EditStorageListAdapter editStorageListAdapter = new EditStorageListAdapter(this);
	...
	{		
		if(LocalStorageActivity.FLAG_SAMBA.equalsIgnoreCase(m_strCurLocation))
		{
			...
			{
				strTitle = this.getResources().getString(R.string.m100_storage_main_media_box);
				iamgeId = R.drawable.m100_media_box_grey;				
			}
			else
			{
				strTitle = this.getResources().getString(R.string.m100_storage_main_hard_disc);
	...
	private void onMoveClick() {
		...
		if(LocalStorageActivity.FLAG_LOCAL.equalsIgnoreCase(m_strCurLocation)){	
			intent.putExtra(LocalStorageActivity.FLAG_CURRENT_LOCATION, LocalStorageActivity.FLAG_LOCAL);				
		}else {		
			intent.putExtra(LocalStorageActivity.FLAG_CURRENT_LOCATION, LocalStorageActivity.FLAG_SAMBA);			
		}
	...
	{	
		DialogAutoDismiss dialog = new DialogAutoDismiss(this.getApplicationContext());
		...
		this.finish();
		LocalStorageActivity.m_curDirectory = new File(CPEConfig.getInstance().getDefaultDir());
	}
	...
	/*private void deleteLocalFiles(InquireDialog dlg)
	{
		...
		if(nDeleteFailed != 0) {
			String msgRes = new String();
			if(nDeleteSuss + nDeleteFailed == 1) {
				msgRes = StorageEditActivity.this.getString(R.string.delete_file_fail1);
			}else{
				String str = StorageEditActivity.this.getString(R.string.delete_file_fail2);
				msgRes = String.format(str, nDeleteSuss,nDeleteFailed);
			}
			Toast.makeText(StorageEditActivity.this, msgRes, Toast.LENGTH_SHORT).show();
		}
		if(LocalStorageActivity.FLAG_LOCAL.equalsIgnoreCase(m_strCurLocation)) {
	...
	private void deleteLocalFiles(InquireDialog dlg)
	{
		LocalDeleteFilesTask task = new LocalDeleteFilesTask(m_localDeleteFilesTaskHandler);
		task.start();	
		dlg.closeDialog();			
		m_progress_dialog = ProgressDialog.show(this, null, this.getString(R.string.deleting),
				true, false);
	}
	
	private void deleteSambaFiles(InquireDialog dlg)
	{
		SmbDeleteFilesTask task = new SmbDeleteFilesTask(m_editStorageListData, m_smbDeleteFilesTaskHandler);
		task.start();	
		dlg.closeDialog();			
		m_progress_dialog = ProgressDialog.show(this, null, this.getString(R.string.deleting),
				true, false);
	...
	//	final InputDialog1 renameFolderDlg = new InputDialog1(this);
		...
		m_renameFolderDlg.m_inputEdit.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,int count) {
				...
				String editable = m_renameFolderDlg.m_inputEdit.getText().toString();
...
/*	
	private boolean isFileNameEmpty(String strName)	
	{			
		boolean bRes = false;
		String dstName = "";	
		...
		if(nIndex >= 0)
		{
			dstName = strName.substring(0, nIndex);			
		}
		else {
			dstName = strName;
		}
		
		if(dstName.trim().length() == 0)
			bRes = true;
		
		return bRes;		
	}*/
	
	private void renameLocalFile(String srcPath, String dstName, InputDialog1 renameFolderDlg)
	{
		File srcFile = new File(srcPath);		
		File file = new File(m_curDirectory + "/" + dstName);
		if(file.exists()) {
			String msgRes = StorageEditActivity.this.getString(R.string.rename_is_exist);
			Toast.makeText(StorageEditActivity.this, msgRes, Toast.LENGTH_SHORT).show();
		}else{
			if(srcFile.renameTo(file) == true) {
				...
				String msgRes = StorageEditActivity.this.getString(R.string.rename_failed);
    			Toast.makeText(StorageEditActivity.this, msgRes, Toast.LENGTH_SHORT).show();
			}
		}
	}
	
	private void renameSambaFile(String srcPath, String dstName, InputDialog1 dlg)
	{
		SmbRenameFileTask task = new SmbRenameFileTask(srcPath, dstName, m_smbRenameFileTaskHandler);
		task.start();	
		dlg.closeDialog();		
	}
	...
	private void onSeleckAllClick() {
		...
		((EditStorageListAdapter)m_editStorageListView.getAdapter()).notifyDataSetChanged();
		showToolbarBtn();
	}
...
/*	private class SortList implements Comparator {
		public int compare(Object o1, Object o2) {
			File c1 = (File) o1;
			File c2 = (File) o2;
			if(c1.isDirectory() == true && c2.isDirectory() == true) {
				return c1.getName().compareToIgnoreCase(c2.getName());
			}
			
			if(c1.isDirectory() == true && c2.isDirectory() == false) {
				return -1;
			}
			
			if(c1.isDirectory() == false && c2.isDirectory() == true) {
				return 1;
			}
			
			if(c1.isDirectory() == false && c2.isDirectory() == false) {
				return c1.getName().compareToIgnoreCase(c2.getName());
			}
	...
	private void showSambaDirectoryLayout() {
		...
		if(bIsRoot == true) {
			...
			m_dirTv.setVisibility(View.VISIBLE);
			m_arrowImage2.setVisibility(View.VISIBLE);
			m_dirParentTv.setText(FileUtils.getFileName(m_parentSmabaDirectory));
			m_dirTv.setText(FileUtils.getFileName(m_curDirectory));
		}
	}
	...
	private class EditStorageListAdapter extends BaseAdapter{
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {	
				...
				convertView = mInflater.inflate(R.layout.storage_edit_list_adapter,null);	
