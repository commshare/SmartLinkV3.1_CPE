...
public class PinDialog implements OnClickListener, OnKeyListener, TextWatcher {
    ...
    private static PinDialog m_instance;

    public static PinDialog getInstance(Context context) {
        //if (m_instance == null)
        m_instance = new PinDialog(context);

        return m_instance;
    }

    private PinDialog(Context context) {
        m_context = context;
        createDialog();
    }
    ...
    private void createDialog() {
        ...
        m_remain_times = (TextView) m_pin_view.findViewById(R.id.pin_remain_times);
        ...
        Button cancelBtn = (Button) m_pin_view.findViewById(R.id.pin_close_btn);
    ...
    private void apply() {
        ...
        String strMsg = m_context.getString(R.string.IDS_PIN_CHECK_PIN_PROGRESS);
        strMsg += "...";
        ...
        BusinessManager.getInstance().sendRequestMessage(MessageUti.SIM_UNLOCK_PIN_REQUEST, data);
    }
    ...
    public void onSimStatusReady(SimStatusModel sim_status) {
        //
        if (null != sim_status) {
            if (sim_status.m_SIMState == SIMState.PinRequired || !m_last_pin_success) {
    ...
    private String formatRemainTimes(int nTimes) {
        String format = this.m_context.getResources().getString(R.string.remain_times);
