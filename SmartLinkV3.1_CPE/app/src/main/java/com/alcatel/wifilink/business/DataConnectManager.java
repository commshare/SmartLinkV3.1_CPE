...
import com.alcatel.wifilink.httpservice.HttpAccessLog;
...
public class DataConnectManager {
    ...
    public void setNoConnected() {
        synchronized (this) {
            ...
            if (m_bPreMobileConnected || m_bPreWifiConnected || m_bPreCPEWifiConnected) {
    ...
    private void sendConnectChangeMsg() {
        if (m_bPreMobileConnected != m_bMobileNetworkConnected || m_bPreWifiConnected != m_bWifiConnected || m_bPreCPEWifiConnected != m_bCPEWifiConnected) {
            Intent intent = new Intent(MessageUti.CPE_WIFI_CONNECT_CHANGE);
            m_context.sendBroadcast(intent);
        }
    }
