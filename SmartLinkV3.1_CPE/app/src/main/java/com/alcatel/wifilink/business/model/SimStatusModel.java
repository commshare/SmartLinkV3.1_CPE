...
public class SimStatusModel extends BaseModel {
    ...
    public boolean equalTo(SimStatusModel instanse) {
        boolean bEqual = true;
        if (!m_strImsi.equalsIgnoreCase(instanse.m_strImsi) || !m_strIccid.equalsIgnoreCase(instanse.m_strIccid) || !m_strMSISDN.equalsIgnoreCase(instanse.m_strMSISDN) || m_SIMState != instanse.m_SIMState || m_bPinOperationalState != instanse.m_bPinOperationalState || m_nPinRemainingTimes != instanse.m_nPinRemainingTimes || m_nPukRemainingTimes != instanse.m_nPukRemainingTimes || m_PinState != instanse.m_PinState)
            bEqual = false;
