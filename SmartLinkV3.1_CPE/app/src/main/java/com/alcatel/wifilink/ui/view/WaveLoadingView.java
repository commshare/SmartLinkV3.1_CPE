...
public class WaveLoadingView extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mCenterTitleUnitPaint.setTextSize(62);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mWaveShader != null) {
            ...
            mShaderMatrix.setScale(1, mAmplitudeRatio / DEFAULT_AMPLITUDE_RATIO / 1.5f, 0, mDefaultWaterLevel);
            ...
            if (!TextUtils.isEmpty(mCenterTitle)) {
                ...
                if (mCenterNum < 10) {
                    ...
                    setCenterText(canvas, 176, 62, mCenterTitle, "", middle, 0.5, 1.05);
                } else if (mCenterNum >= 100 && mCenterNum < 1000) {
                    setCenterText(canvas, 176, 62, mCenterTitle, "", middle, 0.5, 1.55);
                } else {
                    setCenterText(canvas, 176, 62, mCenterTitle, "", middle, 0.5, 2.75);
                }
            }
    ...
    private void setCenterText(Canvas canvas, int titleSize, int unitSize, String mCenterTitle, String unitValue, float middle, double titleWidthRatio, double unitWidthRatio) {
        ...
        canvas.drawText(mCenterTitle, (float) ((getWidth() - middle) * titleWidthRatio), getHeight() / 2 - ((mCenterTitleStrokePaint.descent() + mCenterTitleStrokePaint.ascent()) / 2), mCenterTitleStrokePaint);
        // Draw the centered text
        canvas.drawText(mCenterTitle, (float) ((getWidth() - middle) * titleWidthRatio), getHeight() / 2 - ((mCenterTitlePaint.descent() + mCenterTitlePaint.ascent()) / 2), mCenterTitlePaint);

        mCenterTitleUnitPaint.setTextSize(unitSize);
        // Draw the centered text
        canvas.drawText(unitValue, (float) ((getWidth() - middle) * unitWidthRatio), getHeight() / 2 - ((mCenterTitlePaint.descent() + mCenterTitlePaint.ascent()) / 2), mCenterTitleUnitPaint);
    }
    ...
    private void updateWaveShader() {
        ...
        if (bitmapBuffer == null || haveBoundsChanged()) {
            ...
            if (width > 0 && height > 0) {
                ...
                wavePaint.setColor(adjustAlpha(mWaveColor, 0.7f));
                ...
                wavePaint.setColor(adjustAlpha(mWaveColor, 0.7f));
                final int wave2Shift = (int) (defaultWaveLength / 4);//波浪2相对于波浪1的偏移量
                for (int beginX = 0; beginX < endX; beginX++) {
