...
//				return;
	...
	private String getNotificationContent(ALERT_TYPE type, long used)
	{
		...
		case UsageLimit:
			...
			{
				strContent = String.format(this.getResources().getString(R
									         .string.usage_limit_less_notification_content), CommonUtil.ConvertTrafficMB(this, (long)(settings.HMonthlyPlan/10)));
			}
			break;	
		...
		case BatteryLimit:	
			if(used <= 10)
			{
				strContent = String.format(this.getResources().getString(R.string.battery_limit_notification_content), used);
			}else
			{
				strContent = String.format(this.getResources().getString(R
									         .string.battery_limit_notification_content), used);
			}
		break;
