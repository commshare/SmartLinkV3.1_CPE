package com.alcatel.wifilink.appwidget.waveprogress;
...
public class WaveLoadingView extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        attributes = context.obtainStyledAttributes(attrs, R.styleable.WaveLoadingView, defStyleAttr, 0);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mWaveShader != null) {
            ...
            if (!TextUtils.isEmpty(mBottomTitle)) {
                ...
                // 加入自动换行设置
                // TextPaint mBottomTitlePaint = new TextPaint(this.mBottomTitlePaint);

                // int width = (int) mBottomTitlePaint.measureText(mBottomTitle);
                // Layout.Alignment alignNormal = Layout.Alignment.ALIGN_NORMAL;
                // int spacingmult = 1;
                // int spacingadd = 0;
                // boolean includepad = true;
                // StaticLayout sl = new StaticLayout(mBottomTitle, mBottomTitlePaint, width, alignNormal, spacingmult, spacingadd, includepad);

                float bottom = this.mBottomTitlePaint.measureText(mBottomTitle);
