...
//    	return total;
	...
	private void getUsageHistorySingle() {
		...
		Date now = new Date();
		String strCurrenttime = sDate.format(now);
		
		if(simStatus.m_SIMState != ENUM.SIMState.Accessable) 
			return;
		HttpRequestManager.GetInstance().sendPostRequest(new HttpUsageHistory.GetUsageRecord("7.1",strCurrenttime, new IHttpFinishListener() {           
            @Override
			public void onHttpRequestFinish(BaseResponse response) 
            {   
            	String strErrcode = new String();
